# frozen_string_literal: true

module Ant
  class NotificationComponent < ViewComponent::Base
    attr_reader :message, :description, :type, :duration, :placement, :show_icon, :closable

    def initialize(
      message:,
      description: nil,
      type: "info",
      duration: 4500,
      placement: "topRight",
      show_icon: true,
      closable: true,
      **html_options
    )
      @message = message
      @description = description
      @type = type # 'success', 'info', 'warning', 'error'
      @duration = duration # 毫秒，0 表示不自动关闭
      @placement = placement # 'topLeft', 'topRight', 'bottomLeft', 'bottomRight'
      @show_icon = show_icon
      @closable = closable
      @html_options = html_options
    end

    private

    def notification_classes
      [
        "ant-notification-notice",
        "ant-notification-notice-closable",
        "ant-notification-notice-#{type}",
        "mb-4 last:mb-0 p-4 bg-white rounded-lg shadow-xl border border-gray-200 min-w-[384px] max-w-md pointer-events-auto",
        @html_options[:class]
      ].compact.join(" ")
    end

    def notification_data
      {
        controller: "ant--notification",
        ant__notification_duration_value: duration,
        ant__notification_placement_value: placement
      }
    end

    def icon_color_class
      {
        "success" => "text-green-500",
        "info" => "text-blue-500",
        "warning" => "text-yellow-500",
        "error" => "text-red-500"
      }[type] || "text-blue-500"
    end

    def icon_svg
      icons = {
        "success" => '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>',
        "info" => '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>',
        "warning" => '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>',
        "error" => '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path></svg>'
      }
      icons[type]&.html_safe || icons["info"].html_safe
    end

    def close_icon_svg
      '<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>'.html_safe
    end
  end
end
